...
void main() {
  group('FirebaseAuthService - userStream', () {
    ...
    const logStart = '[userStream] Novo evento de autenticação';
    const logError = '[userStream] Erro ao mapear usuário';
    ...
    group('casos de sucesso', () {
      test(
        ...
        // Act
        final result = await authService.userStream.first;
        ...
        verify(mockLogger.info(argThat(contains(logStart)))).called(1);
      ...
      test(
        ...
        verify(mockFirebaseAuth.authStateChanges()).called(1);
        verify(mockLogger.info(argThat(contains(logStart)))).called(2);
    ...
    group('casos de erro', () {
      test('deve retornar AuthException quando toUserAuth lançar erro',
          () async {
        when(mockUser.toUserAuth()).thenThrow(Exception('Erro de mapeamento'));
        userController.add(mockUser);
